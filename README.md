...
### Немного про сервисы
Те методы, которые необходимо было реализовать по техническому заданию, вынудили описать агрегат(ChatService), поэтому chat service зависит и от репозитория chat, и репозитория message в том числе.
Однако, если мы хотим реализовать уже более узкие методы, например удаление или редактирование сообщения, то есть такие методы, для которых о chat знать не нужно, то мы можем реализовать их в message service(именно поэтому я всё таки оставил его, хотя в рамках задачи он не нужен)
